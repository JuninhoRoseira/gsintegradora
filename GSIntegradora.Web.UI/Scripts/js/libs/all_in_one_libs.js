(function ($) {
	$.fn.getBounds = function () {
		...
		this.each(function (i, el) {
			...
			bounds.zIndex = elQ.css("z-index");

			if (typeof (bounds.zIndex) == "undefined" || bounds.zIndex == "auto") {
				bounds.zIndex = 999;
			}
...
(function (d, c, a, e) {
	var b = {
		...
		addNavigation: function (i) {
			...
			else h.$sliderId.before(f); if (h.options.mobileNavigation) {
		...
		build: function () {
			...
			else f.options.mobileNavigation = false; if (f.options.dynamicArrows) f.addArrows();
			else {
...
(function ($) {
	...
	$.fn.tipsy = function (options) {
		...
		function get(ele) {
			var _tipsy = $.data(ele, "tipsy");
			if (!_tipsy) {
				_tipsy = new Tipsy(ele, $.fn.tipsy.elementOptions(ele, options));
				$.data(ele, "tipsy", _tipsy);
			}
			return _tipsy;
		}
...
if (jQuery) (function (d) {
	...
	function l(b) {
		...
		if (!0 == b.horizontal) {
			...
			a = b.$content.scrollTop(), 0 > a && (a = 0), a /= b.scrollRatio, a > b.handleBounds.bottom && (a = b.handleBounds.bottom), b.$handle.css({
				top: a
			});
	}
	...
	function p(a, b, f) {
		...
		if (!c.hasClass("disabled")) {
			if (!e || b.links) {
				...
				c.addClass("selected")
			} else b.multiple && (b.$optionEls.eq(a).prop("selected", null), c.removeClass("selected")); if (!e || b.multiple) b.callback.call(b.$selecter, b.$selectEl.val(), a), b.index = a
		}
	}
...
(function (e) {
	function n(n, r, i, s) {
		...
		function g() {
			...
			} else o.pauseIcon = undefined; if (o.pagination !==
...
(function (e) {
	var t = function (t, n) {
		...
		else i.currentImage = e(o[i.currentSlide]).find("img:first"); if (e(o[i.currentSlide]).is("a")) e(o[i.currentSlide]).css("display", "block");
		...
		var d = function (t, n, r, i) {
			...
			else s.currentImage = e(n[s.currentSlide]).find("img:first"); if (r.controlNav) {
				e("a",
...
var Swiper = function (selector, params) {
	...
	function handleMousewheel(e) {
		...
		if (e.detail) delta = -e.detail;
			...
			else delta = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? -e.deltaX : -e.deltaY; if (!params.freeMode) {
				if ((new Date).getTime() -
					lastScrollTime > 60)
	...
	function preventClick(e) {
		if (!_this.allowLinks) {
			if (e.preventDefault) e.preventDefault();
			else e.returnValue = false; if (params.preventLinksPropagation && "stopPropagation" in
                e) e.stopPropagation();
			return false
		}
	}
	...
	function onTouchMove(event) {
		...
		if (!isTouchEvent || event.touches.length === 1) {
			...
			} else _this.positions.current = _this.positions.start; if (params.freeMode || params.watchActiveIndex) _this.updateActiveSlide(_this.positions.current);
			if (params.grabCursor) {
	...
	function onTouchEnd(event) {
		...
		} else targetSlideSize = slideSize * params.slidesPerView; if (direction === "toNext" && timeDiff >
            300)
	...
	_this.swipeReset = function () {
		...
		} else newPosition = currentPosition < 0 ? Math.round(currentPosition / groupSize) * groupSize : 0; if (params.scrollContainer) newPosition = currentPosition < 0 ? currentPosition : 0;
		if (newPosition < -maxWrapperPosition()) newPosition = -maxWrapperPosition();
	...
	_this.swipeTo = function (index, speed, runCallbacks) {
		...
		else newPosition = -index * slideSize; if (newPosition < -maxWrapperPosition()) newPosition = -maxWrapperPosition();
		if (newPosition === currentPosition) return false;
	...
	_this.updateActiveSlide =
        function (position) {
        	...
        	} else _this.activeIndex = Math[params.visibilityFullFit ? "ceil" : "round"](-position / slideSize); if (_this.activeIndex === _this.slides.length) _this.activeIndex = _this.slides.length - 1;
        	...
        	} else _this.activeLoopIndex = _this.activeIndex; if (params.pagination) _this.updatePagination(position)
        };
	...
	_this.createLoop = function () {
		...
		else _this.loopedSlides = params.slidesPerView + params.loopAdditionalSlides; if (_this.loopedSlides > _this.slides.length) _this.loopedSlides = _this.slides.length;
		var slideFirstHTML = "",
            slideLastHTML = "",
            i;
	...
	function makeSwiper() {
		...
		else _this.updateActiveSlide(0); if (params.autoplay) _this.startAutoplay();
	...
	getWrapperTranslate: function (axis) {
		...
		if (this.support.transforms && this.params.useCSS3Transforms) {
			...
			if (window.WebKitCSSMatrix) transformMatrix = new WebKitCSSMatrix(curStyle.webkitTransform);
				...
				else curTransform = parseFloat(matrix[4]); if (axis === "y")
